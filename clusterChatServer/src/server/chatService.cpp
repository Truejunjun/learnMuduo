...
{
    lock_guard<mutex> lock(_connMutex);     // 防止执行回调的时候有人下线了，导致Map中去除了user
    auto it = _userConnMap.find(userid);    // 由于是服务器亲自订阅的，所以接收消息的服务器铁有该id
    if (it != _userConnMap.end())
    {
        it->second->send(msg);
        return;
